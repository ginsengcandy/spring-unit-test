...
public class OrderServiceSupportTest {
    ...
    void 성공_주문_생성() {
        ...
        // when
        List<OrderLine> result = OrderServiceSupport.buildOrderLines(order, productList, orderLines);

        // then
        assertThat(result.size()).isEqualTo(3);

        // product 1 - 티셔츠
        assertThat(result.get(0).getProduct().getAmount()).isEqualTo(90L); // 재고
        assertThat(result.get(0).getProduct().getSaleCount()).isEqualTo(10L); // 주문수량
        // product 2 - 바지
    ...
    void 실패_존재하지_않는_상품_주문() {
        ...
        RuntimeException e = assertThrows(RuntimeException.class,
                () -> OrderServiceSupport.buildOrderLines(order, productList, orderLines));
